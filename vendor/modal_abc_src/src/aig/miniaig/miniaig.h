...
static void Mini_AigGrow( Mini_Aig_t * p, int nCapMin )
{
    ...
    assert( p->pArray );
    p->pPosX = MINI_AIG_REALLOC( float, p->pPosX, nCapMin ); 
    p->pPosY = MINI_AIG_REALLOC( float, p->pPosY, nCapMin ); 
    assert( p->pPosX );
    assert( p->pPosY );
    p->nCap   = nCapMin;
}
...
static Mini_Aig_t * Mini_AigStart()
{
    ...
    p->pPosX = MINI_AIG_ALLOC( float, p->nCap );
    p->pPosY = MINI_AIG_ALLOC( float, p->nCap );
...
static void Mini_AigStop( Mini_Aig_t * p )
{
    ...
    MINI_AIG_FREE( p->pPosX );
    MINI_AIG_FREE( p->pPosY );
    MINI_AIG_FREE( p );
}

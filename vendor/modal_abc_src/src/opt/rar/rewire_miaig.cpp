...
int Miaig::fromMiniAig(Mini_Aig_t *pMiniAig) {
    ...
    // // initialize the array of position info
    // if (!_data->pPosX)
    //     _data->pPosX = (float *)malloc(sizeof(float) * nObjs());
    // if (!_data->pPosY)
    //     _data->pPosY = (float *)malloc(sizeof(float) * nObjs());
    // Miaig_ForEachObj(i)
    // {
    //     _data->pPosX[i] = pMiniAig->pPosX[2 * i];
    //     _data->pPosY[i] = pMiniAig->pPosY[2 * i];
    // }
...
Miaig Miaig::dup(int fRemDangle, int fMapped) {
    ...
    // // initialize the array of position info
    // if (!pNew._data->pPosX)
    //     pNew._data->pPosX = (float *)malloc(sizeof(float) * 10000);
    // if (!pNew._data->pPosY)
    //     pNew._data->pPosY = (float *)malloc(sizeof(float) * 10000);
    // Miaig_ForEachInput(i)
    // {
    //     objCopy(i) = i;
    //     pNew._data->pPosX[i] = _data->pPosX[i];
    //     pNew._data->pPosY[i] = _data->pPosY[i];
    // }
    // Miaig_ForEachNodeOutput(i) {
    //     assert(objFaninNum(i) > 0);
    //     if (fRemDangle && objTravId(i) != nTravIds())
    //         continue;
    //     objCopy(i) = pNew.appendObj();
    //     pNew._data->pPosX[objCopy(i)] = _data->pPosX[i];
    //     pNew._data->pPosY[objCopy(i)] = _data->pPosY[i];
    //     Miaig_ForEachObjFanin(i, iLit, k)
    //         pNew.appendFanin(objCopy(i), Rw_Lit2LitV(_data->pCopy, iLit));
    // }
    // if (_data->pNtkMapped && fMapped)
    ...
    // for (int j = 1; j < _data->nObjs; j++)
    // {
    //     // std::cout << j << ' ';
    //     assert(_data->pPosX[j] != 0 || _data->pPosY[j] != 0);
    // }

    // for (int j = 1; j < pNew._data->nObjs; j++)
    //     assert(pNew._data->pPosX[j] != 0 || pNew._data->pPosY[j] != 0);
...
void Miaig::dupDfs_rec(Miaig &pNew, int iObj) {
    ...
    // pNew._data->pPosX[objCopy(iObj)] = _data->pPosX[iObj];
    // pNew._data->pPosY[objCopy(iObj)] = _data->pPosY[iObj];
    // // std::cout << objCopy(iObj) << '*' << iObj << '*';
    // // std::cout << pNew._data->pPosX[objCopy(iObj)] << '&' << pNew._data->pPosY[objCopy(iObj)] << '&';
...
Miaig Miaig::dupDfs(void) {
    ...
    int i; // obj2obj
    // for each primary input we mark it with it's index  // original
    Miaig_ForEachConstInput(i)
        objCopy(i) = i;
    ...
    // Miaig_ForEachOutput(i)
    // {
    //     int newNode = pNew.appendObj();
    //     pNew._data->pPosX[newNode] = _data->pPosX[i];
    //     pNew._data->pPosY[newNode] = _data->pPosY[i];
    //     pNew.appendFanin(newNode, Rw_Lit2LitV(_data->pCopy, objFanin0(i)));
    // }
    ...
    // {
    //     // std::cout << j << ' ';
    //     // std::cout << pNew._data->pPosX[j] << '&' << pNew._data->pPosY[j] << '&';
    //     assert(pNew._data->pPosX[j] != 0 || pNew._data->pPosY[j] != 0);
    // }
...
int Miaig::buildNodeBalance_rec(Miaig &pNew, vi *vFanins, int begin, int end, int fCprop, int fStrash, float rootX, float rootY) {
    float xPosTmp, yPosTmp;
    int iLit;
    ...
    if (length == 1) {
        ...
        // xPosTmp = (pNew._data->pPosX[Abc_Lit2Var(Rw_Lit2LitL(_data->pCopy, Vi_Read(vFanins, begin)))] + pNew._data->pPosX[Abc_Lit2Var(Rw_Lit2LitL(_data->pCopy, Vi_Read(vFanins, end)))] + rootX) / 3;
        // yPosTmp = (pNew._data->pPosY[Abc_Lit2Var(Rw_Lit2LitL(_data->pCopy, Vi_Read(vFanins, begin)))] + pNew._data->pPosY[Abc_Lit2Var(Rw_Lit2LitL(_data->pCopy, Vi_Read(vFanins, end)))] + rootY) / 3;
        // iLit = pNew.buildNode(Rw_Lit2LitL(_data->pCopy, Vi_Read(vFanins, begin)), Rw_Lit2LitL(_data->pCopy, Vi_Read(vFanins, end)), fCprop, fStrash);
        // pNew._data->pPosX[Abc_Lit2Var(iLit)] = xPosTmp;
        // pNew._data->pPosY[Abc_Lit2Var(iLit)] = yPosTmp;
        // return iLit;
    }
    int middle = begin + length / 2;
    int iLit0 = buildNodeBalance_rec(pNew, vFanins, begin, middle - 1, fCprop, fStrash, rootX, rootY);
    int iLit1 = buildNodeBalance_rec(pNew, vFanins, middle, end, fCprop, fStrash, rootX, rootY);
    return pNew.buildNode(iLit0, iLit1, fCprop, fStrash);
    // iLit = pNew.buildNode(iLit0, iLit1, fCprop, fStrash);
    // pNew._data->pPosX[Abc_Lit2Var(iLit)] = (pNew._data->pPosX[Abc_Lit2Var(iLit0)] + pNew._data->pPosX[Abc_Lit2Var(iLit1)] + rootX) / 3;
    // pNew._data->pPosY[Abc_Lit2Var(iLit)] = (pNew._data->pPosY[Abc_Lit2Var(iLit0)] + pNew._data->pPosY[Abc_Lit2Var(iLit1)] + rootY) / 3;
    // return iLit;
}

int Miaig::buildNodeBalance(Miaig &pNew, vi *vFanins, int fCprop, int fStrash, float rootX, float rootY) {
    return buildNodeBalance_rec(pNew, vFanins, 0, Vi_Size(vFanins) - 1, fCprop, fStrash, rootX, rootY);
}

int Miaig::buildNodeCascade(Miaig &pNew, vi *vFanins, int fCprop, int fStrash, float rootX, float rootY) {
    float xPosTmp, yPosTmp;
    int iLit = -1;
    for (int i = 0; i < Vi_Size(vFanins); ++i) {
        ...
        // {
        //     xPosTmp = (pNew._data->pPosX[Abc_Lit2Var(iLit)] + pNew._data->pPosX[Abc_Lit2Var(Rw_Lit2LitL(_data->pCopy, Vi_Read(vFanins, i)))] + rootX) / 3;
        //     yPosTmp = (pNew._data->pPosY[Abc_Lit2Var(iLit)] + pNew._data->pPosY[Abc_Lit2Var(Rw_Lit2LitL(_data->pCopy, Vi_Read(vFanins, i)))] + rootY) / 3;
        //     iLit = pNew.buildNode(iLit, Rw_Lit2LitL(_data->pCopy, Vi_Read(vFanins, i)), fCprop, fStrash);
        //     pNew._data->pPosX[Abc_Lit2Var(iLit)] = xPosTmp;
        //     pNew._data->pPosY[Abc_Lit2Var(iLit)] = yPosTmp;
        // }
...
Miaig Miaig::dupStrash(int fCprop, int fStrash, int fCascade) {
    ...
    Miaig_ForEachNode(i) {
        ...
        if (fCascade)
            ...
            objCopy(i) = buildNodeCascade(pNew, _data->pvFans + i, fCprop, fStrash, 0, 0);
        else
            // objCopy(i) = buildNodeBalance(pNew, _data->pvFans + i, fCprop, fStrash);
            objCopy(i) = buildNodeBalance(pNew, _data->pvFans + i, fCprop, fStrash, 0, 0);
    }
    Miaig_ForEachOutput(i)
    ...
    //     start = pNew.nObjs();
    //     if (fCascade)
    //         objCopy(i) = buildNodeCascade(pNew, _data->pvFans + i, fCprop, fStrash, _data->pPosX[i], _data->pPosY[i]);
    //     else
    //         objCopy(i) = buildNodeBalance(pNew, _data->pvFans + i, fCprop, fStrash, _data->pPosX[i], _data->pPosY[i]);
    //     end = pNew.nObjs();
    //     // the new root node has the same coordinates as the original one
    //     pNew._data->pPosX[end - 1] = _data->pPosX[i];
    //     pNew._data->pPosY[end - 1] = _data->pPosY[i];
    //     // for ( int j = start; j < end; j++ )
    //     // {
    //     //     pNew._data->pPosX[j] = ((j - start + 1) * _data->pPosX[i] + (end - 1 - j) * pNew._data->pPosX[j]) / (end - start);
    //     //     pNew._data->pPosY[j] = ((j - start + 1) * _data->pPosY[i] + (end - 1 - j) * pNew._data->pPosY[j]) / (end - start);
    //     // }
    // }
    // Miaig_ForEachOutput(i)
    // {
    ...
    //     pNew._data->pPosX[newNode] = _data->pPosX[i];
    //     pNew._data->pPosY[newNode] = _data->pPosY[i];
    //     pNew.appendFanin(newNode, Rw_Lit2LitL(_data->pCopy, objFanin0(i)));
    // }
...
Miaig Miaig::dupMulti(int nFaninMax_, int nGrowth) {
    ...
    memset(_data->pCopy, 0, sizeof(int) * nObjs()); // obj2lit
    Miaig_ForEachConstInput(i) // original
        objCopy(i) = Abc_Var2Lit(i, 0);
    Miaig_ForEachNode(i) {
        if (pStop[i] == 1)
            continue;
    ...
    //             pNew._data->pPosX[iObj] = _data->pPosX[i];
    //             pNew._data->pPosY[iObj] = _data->pPosY[i];
    //             Vi_ForEachEntryStop(vArray, iLit, k, nFaninMaxLocal)
    //             {
    //                 pNew.appendFanin(iObj, iLit);
    //                 pNew._data->pPosX[iObj] += pNew._data->pPosX[Abc_Lit2Var(iLit)];
    //                 pNew._data->pPosY[iObj] += pNew._data->pPosY[Abc_Lit2Var(iLit)];
    //                 // std::cout << pNew._data->pPosX[Abc_Lit2Var(iLit)] << '*' << pNew._data->pPosY[Abc_Lit2Var(iLit)] << "***";
    //             }
    //             pNew._data->pPosX[iObj] /= (k + 1);
    //             pNew._data->pPosY[iObj] /= (k + 1);
    //             // std::cout << k << '*' << nFaninMaxLocal << "***";
    ...
    //         // pNew._data->pPosX[iObj] /= (k + 1);
    //         // pNew._data->pPosY[iObj] /= (k + 1);
    //         // // std::cout << k << '*' << nFaninMaxLocal << std::endl;
...
void Miaig::extractBest(vi *vPairs) {
    ...
    // _data->pPosX[iObj] = (_data->pPosX[Abc_Lit2Var(pArray[iBest])] + _data->pPosX[Abc_Lit2Var(pArray[iBest + 1])]);
    // _data->pPosY[iObj] = (_data->pPosY[Abc_Lit2Var(pArray[iBest])] + _data->pPosY[Abc_Lit2Var(pArray[iBest + 1])]);

    Miaig_ForEachNode(i) // original
        Counter += updateFanins(_data->pvFans + i, pArray[iBest], pArray[iBest + 1], Abc_Var2Lit(iObj, 0));
    // Miaig_ForEachNode(i)
    //     if ( updateFanins(_data->pvFans + i, pArray[iBest], pArray[iBest + 1], Abc_Var2Lit(iObj, 0)) )
    //     {
    //         Counter++;
    //         _data->pPosX[iObj] += _data->pPosX[i];
    //         _data->pPosY[iObj] += _data->pPosY[i];
    //     }
    // _data->pPosX[iObj] /= (Counter + 2);
    // _data->pPosY[iObj] /= (Counter + 2);
...
int Miaig::expandOne(int iObj, int nAddedMax, int nDist, int nExpandableLevel, int fVerbose) {
    ...
    std::stable_sort(pOrderF, pOrderF + Vi_Size(_data->vOrderF), [&](int a, int b) {
        return objLevel(a) > objLevel(b);
    });
    std::stable_sort(pOrderF, pOrderF + Vi_Size(_data->vOrderF), [&](int a, int b) {
        if (objLevel(a) == 0 || objLevel(b) == 0) {
            return false;
        }
        return objRef(a) < objRef(b);
    });
    // std::stable_sort(pOrderF, pOrderF + Vi_Size(_data->vOrderF), [&](int a, int b) {
    //     // if (objLevel(Abc_Lit2Var(a)) == 0 || objLevel(Abc_Lit2Var(b)) == 0) {
    //     //     return false;
    //     // }
    //     float aDiffPosX = objPosX(a) - objPosX(iObj);
    //     float aDiffPosY = objPosY(a) - objPosY(iObj);
    //     float bDiffPosX = objPosX(b) - objPosX(iObj);
    //     float bDiffPosY = objPosY(b) - objPosY(iObj);
    //     // std::cout << objPosX(a) << ' ' << objPosY(a) << ' ' << objPosX(b) << ' ' << objPosY(b) << ' ' << objPosX(iObj) << ' ' << objPosY(iObj) << std::endl;
    //     return aDiffPosX * aDiffPosX + aDiffPosY * aDiffPosY < bDiffPosX * bDiffPosX + bDiffPosY * bDiffPosY;
    // });
...
int Miaig::reduceOne(int iObj, int fOnlyConst, int fOnlyBuffer, int fHeuristic, int fVerbose) {
    ...
    if (fHeuristic) {
        ...
        //     //     return false;
        //     // }
        //     float aDiffPosX = objPosX(Abc_Lit2Var(a)) - objPosX(iObj);
        //     float aDiffPosY = objPosY(Abc_Lit2Var(a)) - objPosY(iObj);
        //     float bDiffPosX = objPosX(Abc_Lit2Var(b)) - objPosX(iObj);
        //     float bDiffPosY = objPosY(Abc_Lit2Var(b)) - objPosY(iObj);
        //     // std::cout << objPosX(Abc_Lit2Var(a)) << ' ' << objPosY(Abc_Lit2Var(a)) << ' ' << objPosX(Abc_Lit2Var(b)) << ' ' << objPosY(Abc_Lit2Var(b)) << ' ' << objPosX(iObj) << ' ' << objPosY(iObj) << std::endl;
        //     return aDiffPosX * aDiffPosX + aDiffPosY * aDiffPosY < bDiffPosX * bDiffPosX + bDiffPosY * bDiffPosY;
        // });
    }
...
Miaig Miaig::rewire(int nIters, float levelGrowRatio, int nExpands, int nGrowth, int nDivs, int nFaninMax, int nTimeOut, int nMode, int nMappedMode, int nDist, int nVerbose) {
    ...
    Cmd_CommandExecute( pAbcLocal, "read_lib ../asap7_clean.lib" );
    ...
    // float (Miaig::*Miaig_ObjectiveFunction)(int, int) = (nMode == 0) ? &Miaig::countHPWL : &Miaig::countTransistors;

...
public:
    ...
    float countHPWL(int reset = 0, int fDummy = 0);
...
private:
    ...
    int buildNodeBalance_rec(Miaig &pNew, vi *vFanins, int begin, int end, int fCprop, int fStrash, float rootX, float rootY);

private:
    int buildNode(int l0, int l1, int fCprop, int fStrash);
    int buildNodeBalance(Miaig &pNew, vi *vFanins, int fCprop, int fStrash, float rootX, float rootY);
    int buildNodeCascade(Miaig &pNew, vi *vFanins, int fCprop, int fStrash, float rootX, float rootY);
...
inline void Miaig::release(void) {
    if (_refcount && RW_XADD(_refcount, -1) == 1) {
        if (_data) {
            ...
            if (_data->pPosX) free(_data->pPosX);
            if (_data->pPosY) free(_data->pPosY);
...
inline float &Miaig::objPosX(int i) {
    return _data->pPosX[i];
}
inline float &Miaig::objPosY(int i) {
    return _data->pPosY[i];
}
...
inline float Miaig::countHPWL(int reset, int fDummy) {
    float hpwl = 0;
    Abc_Ntk_t * pNtk = toNtk();
    Abc_FrameReplaceCurrentNetwork( pAbcLocal, pNtk );
    ...
    pNtk = Abc_FrameReadNtk(pAbcLocal);
    hpwl = pNtk->globalHpwl;
    // Abc_NtkDelete(pNtk);
    return hpwl;
}

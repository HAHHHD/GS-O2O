...
extern Map_Man_t *  Abc_NtkToMap( Abc_Ntk_t * pNtk, double DelayTarget, int fRecovery, float * pSwitching, int fVerbose, int fModified, int fHPWL );
...
***********************************************************************/
Abc_Ntk_t * Abc_NtkMap( Abc_Ntk_t * pNtk, Mio_Library_t* userLib, double DelayTarget, double AreaMulti, double DelayMulti, float LogFan, float Slew, float Gain, int nGatesMin, int fRecovery, int fSwitching, int fSkipFanout, int fUseProfile, int fUseBuffs, int fVerbose, int fModified, int fHPWL )
{
    ...
    // perform the mapping
    pMan = Abc_NtkToMap( pNtk, DelayTarget, fRecovery, pSwitching, fVerbose, fModified, fHPWL );
...
***********************************************************************/
Map_Man_t * Abc_NtkToMap( Abc_Ntk_t * pNtk, double DelayTarget, int fRecovery, float * pSwitching, int fVerbose, int fModified, int fHPWL )
{
    Map_Man_t * pMan;
    Map_Node_t * pNodeMap;
    Vec_Ptr_t * vNodes;
    ...
    pMan = Map_ManCreate( Abc_NtkPiNum(pNtk) + Abc_NtkLatchNum(pNtk) - pNtk->nBarBufs, Abc_NtkPoNum(pNtk) + Abc_NtkLatchNum(pNtk) - pNtk->nBarBufs, fVerbose, fModified, fHPWL );
    if ( pMan == NULL )
        return NULL;
    ...
    Abc_NtkForEachCi( pNtk, pNode, i )
    {
        ...
        Map_NodeSetAigPos( pNodeMap, pNode );
    }
    ...
    Abc_NtkForEachCo( pNtk, pNode, i )
    {
        ...
        pNodeMap = Map_Regular(Map_ManReadOutputs(pMan)[i]);
        Map_NodeSetAigPos( pNodeMap, pNode );
    }
...
//            Mio_LibraryReadName((Mio_Library_t *)Abc_FrameReadLibGen()) );
    ...
    // perform the mapping
    pMan = Abc_NtkToMap( pNtk, -1, 1, NULL, 0, 0, 0 );
    if ( pMan == NULL )
        return NULL;
    if ( !Map_Mapping( pMan ) )
    {

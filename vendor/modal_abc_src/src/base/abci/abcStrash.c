...
void Abc_NodeStrash_withPos_rec( Abc_Aig_t * pMan, Hop_Obj_t * pObj, Abc_Obj_t * pRoot )
{
    extern Abc_Obj_t * Abc_AigAndCreate( Abc_Aig_t *, Abc_Obj_t *, Abc_Obj_t * );
    assert( !Hop_IsComplement(pObj) );
    if ( !Hop_ObjIsNode(pObj) || Hop_ObjIsMarkA(pObj) )
        return;
    Abc_NodeStrash_withPos_rec( pMan, Hop_ObjFanin0(pObj), pRoot ); 
    Abc_NodeStrash_withPos_rec( pMan, Hop_ObjFanin1(pObj), pRoot );
    pObj->pData = Abc_AigAnd( pMan, (Abc_Obj_t *)Hop_ObjChild0Copy(pObj), (Abc_Obj_t *)Hop_ObjChild1Copy(pObj) );
    // pObj->pData = Abc_AigAndCreate( pMan, (Abc_Obj_t *)Hop_ObjChild0Copy(pObj), (Abc_Obj_t *)Hop_ObjChild1Copy(pObj) );
    Abc_Obj_t * child0 = Abc_ObjRegular((Abc_Obj_t *)Hop_ObjChild0Copy(pObj));
    Abc_Obj_t * child1 = Abc_ObjRegular((Abc_Obj_t *)Hop_ObjChild1Copy(pObj));
    // if (Abc_ObjIsNode(child0) && !child0->isRoot)
    //     assert (Abc_ObjFanoutNum(child0) == 1);
    // if (Abc_ObjIsNode(child1) && !child1->isRoot)
    //     assert (Abc_ObjFanoutNum(child1) == 1);
    int nFanouts = Abc_ObjFanoutNum((Abc_Obj_t *)pObj->pData);

    ((Abc_Obj_t *)pObj->pData)->xPos = pRoot->xPos;
    ((Abc_Obj_t *)pObj->pData)->yPos = pRoot->yPos;
    // ((Abc_Obj_t *)pObj->pData)->xPos = ( ((Abc_Obj_t *)pObj->pData)->xPos * nFanouts + pRoot->xPos ) / (nFanouts + 1);
    // ((Abc_Obj_t *)pObj->pData)->yPos = ( ((Abc_Obj_t *)pObj->pData)->yPos * nFanouts + pRoot->yPos ) / (nFanouts + 1);
    
    ((Abc_Obj_t *)pObj->pData)->rawPos = 0;
    assert( !Hop_ObjIsMarkA(pObj) ); // loop detection
    Hop_ObjSetMarkA( pObj );
}
...
*/
    ...
    Abc_NodeStrash_withPos_rec( (Abc_Aig_t *)pNtkNew->pManFunc, Hop_Regular(pRoot), pNodeOld );
    ((Abc_Obj_t *)Hop_Regular(pRoot)->pData)->isRoot = 1;

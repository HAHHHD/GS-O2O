...
#include "replace/simplePlacer.h"
...
static int CmdCommandReplace       ( Abc_Frame_t * pAbc, int argc, char ** argv );
static int CmdCommandIncrePlace    ( Abc_Frame_t * pAbc, int argc, char ** argv );
...
void Cmd_Init( Abc_Frame_t * pAbc )
{
    ...
    Cmd_CommandAdd( pAbc, "Basic", "replace",       CmdCommandReplace,         0 );
    Cmd_CommandAdd( pAbc, "Basic", "increPlace",    CmdCommandIncrePlace,      0 );
...
******************************************************************************/
int CmdCommandReplace( Abc_Frame_t * pAbc, int argc, char **argv )
{   
    bool fSimple = 0;
    bool fFast = 0;
    bool fIncre = 0;
    ...
    while ( ( c = Extra_UtilGetopt( argc, argv, "Nsfiphd" ) ) != EOF )
    {
        switch ( c )
        {
        case 'N':
            ...
            nIter = atoi(argv[globalUtilOptind]);
            globalUtilOptind++;
            if ( nIter < 0 )
                goto usage;
            break;
    ...
    if (fSimple)
    {
        ...
        AbcGlobalPlacer placer(pAbc->pNtkCur);
        ...
        placer.run_global_placement(nIter, fDist);
        
        // // Export results
        // placer.export_placement_results("abc_placement.txt");
    }
    else
        Replace_main(pAbc->pNtkCur, fFast, fIncre, fPrintOnly);
    return 0;

  usage:
    fprintf( pAbc->Err, "usage: replace\n" );
    fprintf( pAbc->Err, "      \t\t use Replace to get the results after placement\n" );
    fprintf( pAbc->Err, "   -f \t\t toggle fast placement [default = %d]\n", fFast );
    fprintf( pAbc->Err, "   -i \t\t toggle incremental placement [default = %d]\n", fIncre );
    fprintf( pAbc->Err, "   -p \t\t toggle print only mode [default = %d]\n", fIncre );
    fprintf( pAbc->Err, "   -h \t\tprint the command usage\n" );
    return 1;
}

int CmdCommandIncrePlace( Abc_Frame_t * pAbc, int argc, char **argv )
{
    int c;
    int order = 2;
    int nSizeMax = 0;
    int fHPWL = 0;
    Extra_UtilGetoptReset();
    while ( ( c = Extra_UtilGetopt( argc, argv, "ONdh" ) ) != EOF )
    {
        switch ( c )
        {
        case 'O':
            ...
            order = atoi(argv[globalUtilOptind]);
            globalUtilOptind++;
            if ( order < 0 )
                goto usage;
            break;
    ...
    incrementalPlacement(pAbc->pNtkCur, order, nSizeMax, fHPWL);
    return 0;

  usage:
    fprintf( pAbc->Err, "usage: increPlace\n" );
    fprintf( pAbc->Err, "      \t\t quick incremental placement\n" );
    fprintf( pAbc->Err, "\t-O num      : The negative base-10 exponent defining the convergence threshold [default = %d]\n", order );  
    fprintf( pAbc->Err, "\t-N num      : The maxmum number of extra nodes to be replaced [default = %d]\n", nSizeMax ); 
    fprintf( pAbc->Err, "\t-d num      : toggle using HPWL-oriented placement [default = %d]\n", fHPWL); 
    return 1;
}

...
void Io_NtkWritePiPoNodes( FILE * pFile, Abc_Ntk_t * pNtk )
{
    ...
    // fprintf( pFile, "%d %d ", termWidth, termHeight );
    fprintf( pFile, "%d %d ", 1, 1 );
    fprintf( pFile, " terminal\n");
    }
...
unsigned Io_NtkWriteNodeGate( FILE * pFile, Abc_Obj_t * pNode )
{
    ...
    return ceil(Mio_GateReadArea(pGate));
}
...
void Io_NtkWriteIntNet( FILE * pFile, Abc_Obj_t * pNet )
{
    ...
    else
    {
        ...
        fprintf( pFile, "%s O : ", Mio_GateReadName((Mio_Gate_t *)pFanin->pData) ); 
        else
    ...
    else
    {
        ...
        if ( Abc_NtkHasMapping(pNet->pNtk) )
            ...
            fprintf( pFile, "%s I : ", Mio_GateReadName((Mio_Gate_t *)pFanout->pData) ); 
        else
            // fprintf( pFile, "name O : " );
            fprintf( pFile, "name I : " );
        }
...
void Io_NtkBuildLayout( FILE * pFileScl, FILE * pFilePl, Abc_Ntk_t * pNtk, double aspectRatio, double whiteSpace, unsigned coreCellArea )
{
    ...
    // numCoreRows = (numCoreRows > ceil(numTerms / 4) + 1) ? numCoreRows : ceil(numTerms / 4) + 1;
    ...
    // layoutWidth = (layoutWidth > ceil(numTerms / 4) + 1) ? layoutWidth : ceil(numTerms / 4) + 1;
...
void Io_NtkWritePl( FILE * pFile, Abc_Ntk_t * pNtk, unsigned numTerms, double layoutHeight, double layoutWidth )
{

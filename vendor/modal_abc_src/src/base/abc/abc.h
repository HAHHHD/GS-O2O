...
struct Abc_Obj_t_     // 48/72 bytes (32-bits/64-bits)
{
    ...
    float             arrivalG;
    float             reqG;
    int               isRoot;        // if it's the root of a cell/lut
    int               marked;        // marked leaf during rewriting
    int               rawPos;        // the position will be refined
    float             xPosTmp;
    float             yPosTmp;
    float             rectLx;
    float             rectRx;
    float             rectLy;
    float             rectRy;
    float             half_den_sizeX;
    float             half_den_sizeY;
    float             den_scal;
    float             xPos;          // x coordinate in placement
    float             yPos;          // y coordinate in placement
...
struct Abc_Ntk_t_ 
{
    ...
    int               binDimY;
    float             halfBinRatio;
    float             binModuleRatio;// total module area / total bin area
    int               nBins;         // number of bins
    float *           binsDensity;   // density of bins
    float             invBinArea;
    float             globalHpwl;
    float             timingGScale;  // geometric delay scale per topological level
    // general information 
...
extern ABC_DLL float              Abc_NodeMffcLabelAigHPWL( Abc_Obj_t * pNode, float * overflowSaved, int fClustering );
...
extern ABC_DLL int                Abc_NtkRefactor2( Abc_Ntk_t * pNtk, int nNodeSizeMax, int nMinSaved, int nConeSizeMax, int  fUpdateLevel, int  fUseZeros, int  fUseDcs, int  fVerbose, int fPlace, int fClustering );
/*=== abcRewrite.c ==========================================================*/
...
extern ABC_DLL int                Abc_NtkRewrite2( Abc_Ntk_t * pNtk, int fUpdateLevel, int fUseZeros, int fVerbose, int fVeryVerbose, int fPlaceEnable, int fIndexStart, int fIndexEnd, int fPlace, int fDeep, int fSimple, int fClustering );
extern ABC_DLL int                incrementalPlacement( Abc_Ntk_t * pNtk, int order, int nSizeMax, int fHPWL );
/*=== abcSat.c ==========================================================*/

...
void Abc_AigReplace_int( Abc_Aig_t * pMan, Abc_Obj_t * pOld, Abc_Obj_t * pNew, int fUpdateLevel )
{
    ...
    // printf("%d * %d * %2f * %2f\n", pOld->Id, pOld->Type, pOld->xPos, pOld->yPos);
    // Abc_ObjForEachFanout( Abc_ObjRegular( pNew ), pFanout, v )
    //     printf("\n%d ** %d ** %2f ** %2f\n", pFanout->Id, Abc_ObjFanoutNum(pFanout), pFanout->xPos, pFanout->yPos);
    ...
    {   
        ...
        if ( pNtk->binsDensity )
        {
            xMin = pFanout->xPos - pFanout->half_den_sizeX;
            yMin = pFanout->yPos - pFanout->half_den_sizeY;
            xMax = pFanout->xPos + pFanout->half_den_sizeX;
            yMax = pFanout->yPos + pFanout->half_den_sizeY;
            ...
            for(int xIdx = xb0; xIdx <= xb1; xIdx++) {
                for(int yIdx = yb0; yIdx <= yb1; yIdx++) {
                    pNtk->binsDensity[xIdx * pNtk->binDimY + yIdx] -= xShare[xIdx - xb0] * yShare[yIdx - yb0] * pNtk->invBinArea * pFanout->den_scal;
                }
            }
        }
        ...
        // printf("%d, %.2f, %.2f !\n", pFanout->Id, pFanout->xPos, pFanout->yPos);
        ...
        pFanout->half_den_sizeX = (Abc_ObjFaninC0(pFanout) && Abc_ObjFaninC1(pFanout)) ? 1 : 1.5;
        pFanout->half_den_sizeY = 0.5;
        // SQRT2 = smoothing parameter for density_size calculation
        if(pFanout->half_den_sizeX < pNtk->binStepX / 2 * 1.414213562373095048801) {
            scaleX = pFanout->half_den_sizeX / (pNtk->binStepX / 2 * 1.414213562373095048801);
            pFanout->half_den_sizeX = pNtk->binStepX / 2 * 1.414213562373095048801;
        }
            else {
            scaleX = 1.0;
        }

        if(pFanout->half_den_sizeY < pNtk->binStepY / 2 * 1.414213562373095048801) {
            scaleY = pFanout->half_den_sizeY / (pNtk->binStepY / 2 * 1.414213562373095048801);
            pFanout->half_den_sizeY = pNtk->binStepY / 2 * 1.414213562373095048801;
        }
            else {
            scaleY = 1.0;
        }
        pFanout->den_scal = scaleX * scaleY;
        // binDensity change
        if ( pNtk->binsDensity )
        {
            xMin = pFanout->xPos - pFanout->half_den_sizeX;
...
void Abc_AigDeleteNode( Abc_Aig_t * pMan, Abc_Obj_t * pNode )
{
    ...
    // printf("%d, %.2f, %.2f &&.", pNode->Id, pNode->xPos, pNode->yPos);
    // printf("%d * %d * %d , ", pNode->Id, pNode0->Id, pNode1->Id);

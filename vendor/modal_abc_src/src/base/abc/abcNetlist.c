...
Abc_Ntk_t * Abc_NtkToNetlist( Abc_Ntk_t * pNtk )
{
    ...
    if ( Abc_NtkIsStrash(pNtk) )
    {
        ...
        Abc_NtkForEachLatch(pNtk, pNode, i)
            aa++;
        Abc_NtkForEachLatch(pNtkTemp, pNode, i)
            bb++;
        Abc_NtkForEachLatch(pNtkNew, pNode, i)
            cc++;

        Abc_NtkForEachNode(pNtk, pNode, i)
            pNode->pCopy = pNode->pCopy->pCopy;
        Abc_NtkForEachLatch(pNtk, pLatch, i)
            pLatch->pCopy = pLatch->pCopy->pCopy;
        Abc_NtkForEachPi(pNtk, pi, i)
            pi->pCopy = pi->pCopy->pCopy;
        Abc_NtkForEachPo(pNtk, po, i)
            po->pCopy = po->pCopy->pCopy;

        printf("Nodes: %d, %d, %d\n", a, b, c);
        printf("Latches: %d, %d, %d\n", aa, bb, cc);
...
Abc_Ntk_t * Abc_NtkAigToLogicSop( Abc_Ntk_t * pNtk )
{
    ...
    Abc_Ntk_t * pNtkNew; 
    Abc_Obj_t * pObj, * pFanin, * pNodeNew, * pFanout;
    Vec_Int_t * vInts;
    int i, k, fChoices = 0;
    assert( Abc_NtkIsStrash(pNtk) );
    // start the network
    ...
    if ( Abc_ObjFanoutNum(pObj) > 0 )
    {
        ...
        Abc_ObjForEachFanout( pObj, pFanout, k ) // the position of the constant node is the average of its fanouts
        {
            pObj->pCopy->xPos += pFanout->xPos;
            pObj->pCopy->yPos += pFanout->yPos;
            printf("ID = %d, Type = %d, xPos = %.2f, yPos = %.2f\n", pFanout->Id, pFanout->Type, pFanout->xPos, pFanout->yPos);
        }
        pObj->pCopy->xPos /= Abc_ObjFanoutNum(pObj);
        pObj->pCopy->yPos /= Abc_ObjFanoutNum(pObj);
        // pObj->pCopy->xPos = pObj->xPos;
        // pObj->pCopy->yPos = pObj->yPos;
        pObj->pCopy->rawPos = 0;
        printf("A constant node is added! Type = %d, xPos = %.2f, yPos = %.2f\n", pObj->pCopy->Type, pObj->pCopy->xPos, pObj->pCopy->yPos);
    }

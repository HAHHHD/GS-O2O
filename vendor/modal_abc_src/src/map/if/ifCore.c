...
int If_ManPerformMappingComb( If_Man_t * p )
{
    ...
    // *** Modification ***
    // Extended area oriented mapping
    for ( i = 0; i < p->pPars->nExAreaIters; i++ )
    {  
        int j; 
        If_ManForEachObj( p, pObj, j )
        {
            pObj->nRefsM = pObj->nRefs;
            pObj->nRoots = pObj->nRefs;
            pObj->activated = 0;
        }
        If_ManPerformMappingRound( p, p->pPars->nCutsMax, 3, 0, 0, "ExArea" );
        if ( p->pPars->fExpRed )
            If_ManImproveMapping( p );
    }
    // *** Modification ***

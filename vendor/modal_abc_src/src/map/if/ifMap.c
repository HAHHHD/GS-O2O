...
void If_ObjPerformMappingAnd( If_Man_t * p, If_Obj_t * pObj, int Mode, int fPreprocess, int fFirst )
{
    ...
    // *** Modification ***
    if ( Mode == 3 && pObj->nRefs > 0 )
        If_CutAreaDerefM_Once( p, If_ObjCutBest(pObj) );
    // *** Modification ***
...
//        assert( pCut->Delay <= pObj->Required + p->fEpsilon );
        ...
        if ( Mode == 3 )
        {
            double area = If_CutLutArea(p, pCut), edge = pCut->nLeaves;
            If_CutAreaEdgeDerefedM(p, pCut, &area, &edge);
            pCut->Area = area;
            if (p->pPars->fEdge)
                pCut->Edge = edge;
        }
...
//        p->nBestCutSmall[1]++;
    ...
    // *** Modification ***
    if ( Mode == 3 && pObj->nRefs > 0 )
        If_CutAreaRefM( p, If_ObjCutBest(pObj) ); 
    // *** Modification ***
...
void If_ObjPerformMappingChoice( If_Man_t * p, If_Obj_t * pObj, int Mode, int fPreprocess )
{
    ...
    // *** Modification ***
    if ( Mode == 3 && pObj->nRefs > 0 )
        If_CutAreaDerefM( p, If_ObjCutBest(pObj) );
    // *** Modification ***
...
int If_ManPerformMappingRound( If_Man_t * p, int nCutsUsed, int Mode, int fPreprocess, int fFirst, char * pLabel )
{
    ...
    float arrTime;
    // *** Modification ***
    //assert( Mode >= 0 && Mode <= 2 ); //old version
    assert( Mode >= 0 && Mode <= 3 ); //plus the Mode 3
    // *** Modification ***

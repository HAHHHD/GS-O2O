...
***********************************************************************/
void If_CutAreaDerefM( If_Man_t * p, If_Cut_t * pCut )
{
    If_Obj_t * pLeaf;
    int i;
    If_CutForEachLeaf( p, pCut, pLeaf, i )
    {
        assert( pLeaf->nRefsM > 0 );
        if ( --pLeaf->nRefsM > 0 || !If_ObjIsAnd(pLeaf) )
            continue;
        If_CutAreaDerefM( p, If_ObjCutBest(pLeaf) );
    }
}

void If_CutAreaRefM( If_Man_t * p, If_Cut_t * pCut )
{
    If_Obj_t * pLeaf;
    int i;
    If_CutForEachLeaf( p, pCut, pLeaf, i )
    {
        ...
        if ( pLeaf->nRefsM++ > 0 || !If_ObjIsAnd(pLeaf) )
            continue;
        If_CutAreaRefM( p, If_ObjCutBest(pLeaf) );
    }
}

void If_CutAreaDerefM_Once( If_Man_t * p, If_Cut_t * pCut )
{
    If_Obj_t * pLeaf;
    int i;
    If_CutForEachLeaf( p, pCut, pLeaf, i )
    {
        if ( !pLeaf->activated && If_ObjIsAnd(pLeaf) )
        {
            assert( pLeaf->nRefsM >= pLeaf->nRoots );
            pLeaf->activated = 1;
            pLeaf->nRefsM -= pLeaf->nRoots;
            if ( pLeaf->nRefsM > 0 )
                continue;
            If_CutAreaDerefM( p, If_ObjCutBest(pLeaf) );
        }
...
#include <math.h>
void If_CutAreaEdgeRefM( If_Man_t * p, If_Cut_t * pCut, double * area, double * edge )
{
    If_Obj_t * pLeaf;
    int i;
    If_CutForEachLeaf( p, pCut, pLeaf, i )
    {
        ...
        if ( pLeaf->nRefsM++ > 0 || !If_ObjIsAnd(pLeaf) )
            continue;
        *area += If_CutLutArea(p, If_ObjCutBest(pLeaf)) * pow(0.5, pLeaf->nRefs);// / (double)(pLeaf->nRefs + 1);
        *edge += If_ObjCutBest(pLeaf)->nLeaves * pow(0.5, pLeaf->nRefs);// / (double)(pLeaf->nRefs + 1);
        If_CutAreaEdgeRefM( p, If_ObjCutBest(pLeaf), area, edge );
    }
}

void If_CutAreaEdgeDerefM( If_Man_t * p, If_Cut_t * pCut, double * area, double * edge )
{
    If_Obj_t * pLeaf;
    int i;
    If_CutForEachLeaf( p, pCut, pLeaf, i )
    {
        ...
        *area -= If_CutLutArea(p, If_ObjCutBest(pLeaf)) * pow(0.5, pLeaf->nRefs);// / (double)(pLeaf->nRefs + 1);
        *edge -= If_ObjCutBest(pLeaf)->nLeaves * pow(0.5, pLeaf->nRefs);// / (double)(pLeaf->nRefs + 1);
        If_CutAreaEdgeDerefM( p, If_ObjCutBest(pLeaf), area, edge );
    }
}

void If_CutAreaEdgeDerefedM( If_Man_t * p, If_Cut_t * pCut, double * area, double * edge )
{   
    if ( pCut->nLeaves < 2 )
    {
        *area = 0;
    ...
    double areaTmp = 1;
    double edgeTmp = pCut->nLeaves;
    If_CutAreaEdgeRefM(p, pCut, &areaTmp, &edgeTmp);
    *area = areaTmp;
    *edge = edgeTmp;
    If_CutAreaEdgeDerefM(p, pCut, &areaTmp, &edgeTmp);
    assert( areaTmp > 1 - p->fEpsilon );
    assert( areaTmp < 1 + p->fEpsilon );
    assert( edgeTmp > pCut->nLeaves * (1 - p->fEpsilon ));
    assert( edgeTmp < pCut->nLeaves * (1 + p->fEpsilon ));
}
// *** Modification ***

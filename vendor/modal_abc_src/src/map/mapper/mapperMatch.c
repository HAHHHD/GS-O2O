...
void Map_MatchClean( Map_Match_t * pMatch )
{
    ...
    pMatch->HPWLFlow          = MAP_FLOAT_LARGE; // unassigned
...
//        return 0;
    if ( !fDoingArea )
    {
        if (pMan->fHPWL)
        {
            if ( pM1->HPWLFlow < pM2->HPWLFlow - pMan->fEpsilon )
                return 0;
            if ( pM1->HPWLFlow > pM2->HPWLFlow + pMan->fEpsilon )
                return 1;  
        }
...
int Map_MatchNodeCut( Map_Man_t * p, Map_Node_t * pNode, Map_Cut_t * pCut, int fPhase, float fWorstLimit )
{
    ...
    for ( pSuper = pMatch->pSupers, Counter = 0; pSuper; pSuper = pSuper->pNext, Counter++ )
    {
        ...
        for ( i = 0; i < (int)pSuper->nPhases; i++ )
        {
            ...
            if (p->fHPWL)
                pMatch->HPWLFlow = Map_CutGetHPWLFlow( pNode, pCut, fPhase );
            ...
            else
            {
                ...
                {
                    if ( p->fMappingMode == 2 )
                        pMatch->AreaFlow = Map_CutGetAreaDerefedM( pCut, fPhase ); //*************************
...
//        assert( pCutBest->M[fPhase].tArrive.Fall < pNode->tRequired[fPhase].Fall + p->fEpsilon );
    ...
    if (p->fModified)
    {
        if ( p->fMappingMode == 2 )// || p->fMappingMode == 3 )
        {
            ...
            if ( pNode->nRefAct[fPhase] > 0 || 
                ...
                {   
                    Map_CutDerefM_once( pCutBest, fPhase );
                    Map_CutDeref( pCutBest, fPhase, p->fUseProfile );
                    pMatch->AreaFlow = Area1 = Map_CutGetAreaDerefedM( pCutBest, fPhase );
                }
            else
                pMatch->AreaFlow = Area1 = Map_CutGetAreaDerefedM( pCutBest, fPhase );    //**************************
        }
         ...
         (pNode->pCutBest[!fPhase] == NULL && pNode->nRefAct[!fPhase] > 0)) )
    {
        if (p->fModified)
        {
            if ( p->fMappingMode == 2 )
            {
                Map_CutRef( pNode->pCutBest[fPhase], fPhase, p->fUseProfile );   
                Map_CutRefM_v( pNode->pCutBest[fPhase], fPhase );  //**************************
            }
            else if ( p->fMappingMode == 3 )
...
void Map_NodeTryDroppingOnePhase( Map_Man_t * p, Map_Node_t * pNode )
{
    ...
    if ( fUsePhase0 )
    {
        ...
        if ( p->fMappingMode >= 2 && pNode->nRefAct[1] > 0 )
        {
            ...
            {
                if ( p->fMappingMode == 2 )
                    Map_CutDerefM_v( pNode->pCutBest[1], 1 );
        ...
        if ( p->fMappingMode >= 2 && pNode->nRefAct[0] == 0 )
        {
            ...
            {
                if ( p->fMappingMode == 2 )
                    Map_CutRefM_v( pNode->pCutBest[0], 0 );
    ...
    else
    {
        ...
        if ( p->fMappingMode >= 2 && pNode->nRefAct[0] > 0 )
        {
            ...
            {
                if ( p->fMappingMode == 2 )
                    Map_CutDerefM_v( pNode->pCutBest[0], 0 );
        ...
        if ( p->fMappingMode >= 2 && pNode->nRefAct[1] == 0 )
        {
            ...
            {
                if ( p->fMappingMode == 2 )
                    Map_CutRefM_v( pNode->pCutBest[1], 1 );
    ...
    if (p->fModified)
        if ( p->fMappingMode == 2 ) 
            for ( i = 0; i < p->vMapObjs->nSize; i++ ) //************************ 
            {
                ...
                pNode->activated[0] = pNode->activated[1] = pNode->activated[2] = 0;
                pNode->nRoots[0] = pNode->nRefActM[0] = pNode->nRefAct[0];
                pNode->nRoots[1] = pNode->nRefActM[1] = pNode->nRefAct[1];
                pNode->nRoots[2] = pNode->nRefActM[2] = pNode->nRefAct[2];
            }
    //************************
    for ( i = 0; i < p->vMapObjs->nSize; i++ )
    {
